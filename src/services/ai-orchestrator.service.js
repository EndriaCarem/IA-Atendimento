...
function buildFreeSlotsContext(clinicId, durationMin = 60) {
  ...
  return [
    ...
    ...lines,
    "",
    "OFERECER HORARIOS (CRITICO): quando chegar a hora de marcar, voce mesmo deve MOSTRAR 2 ou 3 horarios REAIS da lista de disponiveis (ex: 'Temos quinta-feira 18/06 as 08h, sexta 19/06 as 09h ou segunda 22/06 as 10h. Qual prefere?'). NUNCA pergunte 'qual data e horario voce prefere?' SEM antes mostrar as opcoes — o paciente nao sabe os horarios livres, quem mostra e voce. So pergunte a preferencia DEPOIS de listar as opcoes reais.",
    "AO OFERECER: mostre no MAXIMO as 3 PRIMEIRAS datas desta lista (ex: as 3 mais proximas). NUNCA liste mais de 3 datas numa mesma mensagem. Use as datas seguintes apenas para entender pedidos como 'proxima semana' ou quando o paciente recusar as primeiras.",
  ].join("\n");
}
...
function buildSystemPrompt({ clinicName, clinicAddress, bookingLink, customPrompt, businessHours, handoff, procedures, insurancePlans, doctors, freeSlotsContext, conversationHistory, stateContext, isKnownPatient = false, knownPatientName = null }) {
  ...
  return [
    ...
    "DATAS (CRITICO): ao oferecer ou confirmar horarios, SEMPRE inclua a DATA COMPLETA no formato 'dia-da-semana DD/MM' (ex: 'quarta-feira 11/06 as 10h'), nunca apenas o nome do dia ('quarta'). Use SOMENTE as datas exatas da lista 'Horarios disponiveis' abaixo — nao diga so 'quarta/quinta/sexta' sem a data. Se o paciente pedir um dia que nao esta na lista (ex: segunda, sabado), diga que nao ha horario nesse dia e ofereca as datas disponiveis reais (com DD/MM).",
