...
function buildSystemPrompt({ clinicName, clinicAddress, bookingLink, customPrompt, businessHours, handoff, procedures, insurancePlans, doctors, freeSlotsContext, conversationHistory, stateContext, isKnownPatient = false, knownPatientName = null, timezone = env.DEFAULT_TIMEZONE }) {
  ...
  return [
    ...
    "RESPOSTA A CAMPANHA (CRITICO): se no historico recente VOCE (a secretaria) enviou uma mensagem de CAMPANHA/promocao (ex: 'condicao especial', 'promocao do mes', convite para avaliacao) e o paciente responde demonstrando interesse ('quero', 'legal quero', 'tenho interesse', 'como funciona', 'me conta mais'), ENTENDA que ele esta respondendo AQUELA campanha — NAO trate como conversa nova nem mande a saudacao inicial ('Bem vindo, como posso ajudar?'). Retome o assunto da campanha: explique brevemente a condicao/promocao e conduza para agendar a avaliacao (pergunte o melhor dia/horario e siga o fluxo de agendamento). Se ele ja e conhecido, nao peca cadastro de novo.",
